...
{
   $data_array = process_no_issue_users ( $data_array, $matchcode_row_index, $project_id, $stat_cols );
}
...
print_tbody ( $data_array, $project_id, $stat_cols, $issue_amount_thresholds, $issue_age_thresholds, $print );
echo '</table>';
echo '</div>';
if ( !$print )
{
...
function print_main_table_head_row ( $dynamic_colspan, $print )
{
   ?>
   <tr>
      <td class="form-title" colspan="<?php echo ( $dynamic_colspan - 1 ); ?>">
         <?php
 ...
 * @param $print
 */
function print_tbody ( $data_rows, $project_id, $stat_cols, $issue_amount_threshold, $issue_age_threshold, $print )
{
   ...
   foreach ( $head_rows_array as $head_row )
   {
      ...
      for ( $group_index = 0; $group_index < count ( $groups[ 0 ] ); $group_index++ )
      {
         ...
         if ( $user_id == $head_row_user_id )
         {
            ...
            $stat_issue_count = print_user_row ( $data_row_index, $data_rows, $stat_cols, $project_id, $issue_amount_threshold, $issue_age_threshold, $stat_issue_count, false, $print );
         }
      }
   }

   $stat_issue_count = process_group ( $groups[ 1 ], $data_rows, $stat_cols, $project_id, $issue_amount_threshold, $issue_age_threshold, $stat_issue_count, 'headrow_no_issue', $print );
   $stat_issue_count = process_group ( $groups[ 2 ], $data_rows, $stat_cols, $project_id, $issue_amount_threshold, $issue_age_threshold, $stat_issue_count, 'headrow_del_user', $print );
   $stat_issue_count = process_group ( $groups[ 3 ], $data_rows, $stat_cols, $project_id, $issue_amount_threshold, $issue_age_threshold, $stat_issue_count, 'headrow_no_user', $print );
...
function print_main_project ( $data_row, $stat_cols, $print )
{
   ...
   if ( access_has_global_level ( $access_level ) && !$print )
   {
      ...
      <a href="search.php?project_id=<?php echo $main_project_id; ?>
                  ...
                  &amp;hide_status_id=-2
                  &amp;match_type=0">
         <?php echo $main_project_name; ?>
      </a>
      <?php
   }
   else
   {
      echo $main_project_name;
   }
   echo '</td>';
}
...
function print_target_version ( $data_row, $stat_cols, $print )
{
   ...
   if ( access_has_global_level ( $access_level ) && !$print )
   {
      ...
      <a href="search.php?project_id=<?php echo $assigned_project_id; ?>
                  ...
                  &amp;hide_status_id=-2
                  &amp;match_type=0">
         <?php echo $target_version_prepared_string; ?>
      </a>
      <?php
   }
   else
   {
      echo $target_version_prepared_string;
   }
   echo '</td>';
}
...
function print_remark ( $data_row, $issue_age_threshold, $stat_cols, $print )
{
   ...
   for ( $stat_index = 1; $stat_index <= get_stat_count (); $stat_index++ )
   {
      ...
      if ( $assigned_project_id == null && $main_project_id == null )
      {
         continue;
      }
